...
//------------------------------------
// ham tao mau chu tai toa do x va y
...
//---------------------------------------------------------------------------------------------------------
// ham tao mau cua cac trang thai cua con ran tai x
...
//-----------------------------------------------------------
//ham tao mau cua cac trang thai cua con ran tai y
...
//-----------------------------------------------------
// ket hop mau cua con ran tai toa do x va y
...
//-------------------------------------------------------
// ham thiet ke giao dien man hinh game ran
void getup(){
    HANDLE hout; 
    ...
    system("code by Sam Van Quan -58131370");
    system("cls");
...
//----------------------------------------------------------
// ham tinh diem cho nguoi choi
void score(int sc){
    gotoxy(69,21);
    printf("%6d",sc*10);
}
//--------------------------
//ham trang thai cua tro choi
...
int main(){
    ...
    while(true){
        if(restart){
            ...
            size=3; // kich thuoc ban dau cua con ran
            speed=200;// thoi gian chay cua con ran
            scr=0;
            score(scr);
            flow=RIGHT;
            xpos[0]=20;
            ...
            for(i=0;i<size;i++){// in ra vi tri cua moi tiep theo
        ...
        char ch=getch();
        switch(tolower(ch)){// neu sua dung phim x thi thoat game
            case 'x' :  system("cls");
                        return 0;
            case ' ' : status("Paused"); 
                        while(true){
                        ...
                        break;
            case -32: { 
            //di chuyen con ran bang cac phim mui ten di chuyen
                char chh=getch();
                if(chh==72 && flow!=DOWN)
                    flow=UP;
